...
class uhd_oz39(gr.top_block):

    def __init__(self, control_ip='127.0.0.1', control_port=8000, rf_freq=750e6, rf_gain=50.0, samp_rate=40e6, zmq_out_ip='127.0.0.1', zmq_out_port=5000):
        gr.top_block.__init__(self, "Uhd Oz39", catch_exceptions=True)
...
def main(top_block_cls=uhd_oz39, options=None):
    if options is None:
        options = argument_parser().parse_args()
    tb = top_block_cls(control_ip=options.control_ip, control_port=options.control_port, rf_freq=options.rf_freq, rf_gain=options.rf_gain, samp_rate=options.samp_rate, zmq_out_ip=options.zmq_out_ip)
